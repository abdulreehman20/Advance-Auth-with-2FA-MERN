...
export const errorHandler: ErrorRequestHandler = (err: unknown, req: Request, res: Response, _next): void => {
	...
	// Handle Mongoose duplicate key errors
		...
		const match = (err as Error & { keyPattern?: Record<string, unknown> }).keyPattern;
		if (match) { details = { duplicateFields: Object.keys(match) }; }
	}
	...
	// Handle JWT errors
		...
		message = err.name === "TokenExpiredError" ? "Your session has expired. Please log in again" : "Invalid authentication token";
	}

	// Handle rate limit errors (from express-rate-limit)
	else if (err instanceof Error && "statusCode" in err && err.statusCode === HTTPSTATUS.TOO_MANY_REQUESTS) {
		statusCode = HTTPSTATUS.TOO_MANY_REQUESTS;
		errorCode = ErrorCodeEnum.RATE_429;
		message = "Too many requests. Please try again later";
	} else if (err instanceof Error && "code" in err && (err.code === "LIMIT_FILE_SIZE" || err.code === "LIMIT_FILE_COUNT" || err.code === "LIMIT_UNEXPECTED_FILE")) {
		statusCode = HTTPSTATUS.BAD_REQUEST;
		errorCode = ErrorCodeEnum.FILE_UPLOAD_ERROR;
		message = "File upload error";
		message = err.code === "LIMIT_FILE_SIZE" ? "File size exceeds the maximum allowed limit" : message;
	}

	// Handle network/connection errors
	else if (err instanceof Error && (err.name === "ECONNREFUSED" || err.name === "ETIMEDOUT" || err.name === "ENOTFOUND")) {
